...
function IsInHexSystem:init()
  self.pool.onAdded = function(_, entity)
    states.in_game.map:addEntityToHex(entity, entity.is_in_hex:fetch(self:getWorld()))
  end
end

function IsInHexSystem:update()
  for _, entity in ipairs(self.pool) do
    local x, y = states.in_game.map:getPixelCoordsFromHex(entity.is_in_hex:fetch(self:getWorld()))
    entity.position.x = x
    entity.position.y = y
  end
end
