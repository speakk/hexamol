...
function MapClickHandlerSystem:handle_map_click(hex)
  ...
  if entity_exists_in_hex then
    if entity_exists_in_hex.is_in_team.team == team and entity_exists_in_hex.can_be_selected then
      self:getWorld():emit("select_entity", entity_exists_in_hex)
    elseif #(self.selected) > 0 and entity_exists_in_hex.is_in_team.team ~= team then
